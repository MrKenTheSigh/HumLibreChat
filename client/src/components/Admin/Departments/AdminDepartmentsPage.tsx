...
import { useGetAdminDepartmentsQuery } from '~/data-provider/Admin';
...
export default function AdminDepartmentsPage() {
  ...
  return (
    <AdminLayout title={localize('com_ui_admin_departments')} hideHeader={true}>
          ...
          <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <AdminHelpButton
                  title="com_ui_admin_departments"
                  description="com_ui_admin_departments_description"
                />
              </div>
            </div>
        ...
        <section className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-3xl border border-border-medium bg-surface-primary">
          <div className="min-h-0 flex-1 overflow-auto px-5 py-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              ...
              <div className="text-sm text-text-secondary">{filteredDepartments.length}</div>
            </div>
            ...
            {departmentsQuery.isLoading ? (
              ...
              <div className="rounded-2xl border border-dashed border-border-medium bg-background p-6 text-sm text-text-secondary">
                {localize('com_ui_admin_empty_departments')}
              </div>
            ) : (
              <div className="flex flex-col gap-3">
                {pagedDepartments.map((department) => (
                  <button
                    ...
                    onClick={() => navigate(`/d/admin/departments/${department.id}`)}
                  >
                    <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                      <div className="space-y-2">
                        <div>
                          ...
                          <div className="text-sm text-text-secondary">{department.code}</div>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          ...
                          />
                          <StatusBadge
                            active={department.parentDepartmentId != null}
                            label={`${localize('com_ui_admin_parent_department')}: ${
                    ...
                    <div className="grid gap-3 text-sm md:grid-cols-3">
                      <div className="rounded-xl border border-border-light bg-surface-primary px-3 py-2">
                        ...
                        <div className="mt-1 text-text-primary">
                          {department.description || localize('com_ui_none')}
                        </div>
                      </div>
                      <div className="rounded-xl border border-border-light bg-surface-primary px-3 py-2">
                        <div className="text-xs uppercase tracking-wide text-text-secondary">
                        ...
                        <div className="mt-1 text-text-primary">{department.sortOrder}</div>
                      </div>
                      <div className="rounded-xl border border-border-light bg-surface-primary px-3 py-2">
                        <div className="text-xs uppercase tracking-wide text-text-secondary">
                        ...
                        <div className="mt-1 text-text-primary">
                          {formatAdminDateTime(department.updatedAt)}
                        </div>
                      </div>
                    </div>
                  </button>
